...
import { CARDS_ENDPOINT, getSetIdArray } from '@nickgdev/larvitar-types';
...
async function collect() {
  ...
  for (const setMeta of getSetIdArray()) {
    const filepath = path.join(pathToJsonDir, standardizeFileName(setMeta.name));
    if (!fileExists(filepath)) {
      /** 2. Create a write stream to a new file utilizing the set name */
      const fileWriteStream = fs.createWriteStream(filepath, { encoding: 'utf-8', emitClose: true });

      /** 3. Ahead is code that may throw an error */
      try {
        /** 4. Create the cards endpoint using setId query string */
        const url = CARDS_ENDPOINT + `?q=set.id:${setMeta.id}`;
        ...
        if (count < totalCount) {
          ...
          while (currentCount < totalCount) {
            ...
            const { data: nextCardSetData, status } = await axios.get(CARDS_ENDPOINT + `?q=set.id:${setMeta.id}&page=${nextPage}`);
          ...
          fileWriteStream.close(() =>
            console.log(`File ${standardizeFileName(setMeta.name)}.json written. Stream closed w closure code 0;`)
          );
        } else {
